...
let drink2: [string, string, number] = ["cola", "콜라", 5];
drink2[0] = "사이다";
...
const student1: Student = { name: "진우", isPassed: true };
...
interface 야구부 extends Student {
  ...
  weight: number;
  height: number;
  readonly backNumber?: number; // 없어도 되는 값 ? 처리리
  [grade: number]: Score;
}
...
// otani.backNumber=11; // readOnly 수정 불가가
...
interface Toy {
  name: string;
  start(): void; // 함수 중 리턴타입이 없을 경우 void 타입입
}
interface Car {
  name: string;
  color: string;
...
// 교차 타입& > Toy와 Car의 정보를 모두 만족해야 함함
let toyCar: Toy & Car = {
  name: "타요",
  color: "blue",
  price: 50000,
