...
const SECRET = "Duf8NG1gjC8aqZaI"; // .env에 저장해서 사용
...
app.post("/login", (req, res) => {
  try {
    ...
    if (id === realId && pw === realPw) {
      //로그인 성공
      //jwt 발급
      const token = jwt.sign({ id }, SECRET); // sign(payload, secretKey) , 키값과 밸류값이 같으면 밸류 생략
      console.log("token:", token);
      // jwt는 클라이언트에서 관리하기 때문에
      // 클라이언트에게 토큰을 보내주어야 함
...
app.post("/token", (req, res) => {
  try {
    console.log(req.headers.authorization); // Bearer  sdfs asdfsfsd. dsdsdssdZ
