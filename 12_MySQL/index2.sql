...
-- 외래키 테이블
-- ON UPDATE CASCADE : 기존 테이블의 데이터가 변경되면 참조 테이블의 데이터도 변경됩니다.
...
-- 회원테이블 (customer) (참조되는 기본키가 있는 테이블) (2) 나중 삭제
...
-- 4. WHERE 로 INNER 조인 사용, 조회 조건 추가
SELECT *
FROM customer,orderlist
...
show GRANTS for 'user2'@'localhost'; -- (권환 확인 ) 권환이 없음..
